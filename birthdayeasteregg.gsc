...
#precache("model", "p7_bezoz_photo_ee_alt");
...
function autoexec birthday()
{
	...
	level.classiclightingfx = "zombie/fx_monkey_lightning_zmb";
	//level._effect["poltergeist"] = "zombie/fx_monkey_lightning_zmb";
	//finalinteractitems
	...
	level.finalinteractmitchelleespot = GetEnt("finalinteractmitchellee", "targetname");
	//level.walterwhiteheadmodglasses = GetEnt("walterwhiteheadglasses", "targetname");
	//level.walterwhiteheadmodglasses hide();
	//
	level.unknownfinaltotemtrig SetHintString("Press ^2[{+activate}]^7 To Place");
	level.a153finaltotemtrig SetHintString("Press ^2[{+activate}]^7 To Place");
	level.bezozfinaltotemtrig SetHintString("Press ^2[{+activate}]^7 To Place");
	...
	thread PlayFxWithCleanup(level._effect["poltergeist"], level.calendartoppage.origin);
	level.calandarbackpageee show();
	level.calendartoppage show();
	level.calendarbottompage.shouldbevisible = false;
	level.calendarbreakspot waittill("trigger", player);
	thread PlayFxWithCleanup("destruct/fx_dest_paper", level.paperfxspot.origin);
	level.calendartoppage Delete();
	level.calendarbottompage show();
	level.calendarbottompage.shouldbevisible = true;
	level.calendarbreakspot Delete();
	wait(1);
	thread PlayFxWithCleanup(level._effect["powerup_grabbed"], level.paperfxspot.origin);
...
function walterwhitehead()
{
	...
	thread PlayFxWithCleanup(level._effect["powerup_grabbed"], level.walterwhiteheadmod.origin);
...
function bdayscene()
{
	...
	//PlayFX(level.newlightingfxaj, level.birthdaycake.origin);
	wait(.5);
	level.activatebowstrig SetHintString("Press ^2[{+activate}]^7 To Receive a Reward [REQUIRES ALL PLAYERS]");
	level.activatebowstrig waittill("trigger", player);
	level.calandarbackpageee Delete();
	level.calendarbottompage Delete();
	level.calendarbottompagealt Delete();
...
function startbdayscene()
{
	while(1)
	{
		...
		{
			//add logic here to prevent teleports during ee centric moments, same with other teleport EEs
			level.secretcompartment PlaySound("vox_153ee_verificationcomplete");
			thread zm_subtitles::subtitle_display(undefined, 2, "-... .- ... . -..", "verification completed.");
			level.finalinteractmitchelleespot UseTriggerRequireLookAt();
			level.finalinteractmitchelleespot SetCursorHint("HINT_NOICON");
			level.finalinteractmitchelleespot SetHintString("Press ^2[{+activate}]^7 To Receive a Reward [REQUIRES ALL PLAYERS]");
			while(1)
			{
				players = GetPlayers();
				playersinrange = 0;
				level.finalinteractmitchelleespot waittill("trigger", player);
				foreach(player in GetPlayers())
				{
					///if(player IsTouching(radius))
					if(Distance(player.origin, level.finalinteractmitchelleespot.origin)<= 115)
					{
						playersinrange ++;
					}
				}
			...
			level.finalinteractmitchelleespot Delete();
			...
			wait 2;
			level.unknownfinaltotem Delete();
			level.a153finaltotem Delete();
			level.bezozfinaltotemtrig Delete();
			level.finalinteractmitchelleespot Delete();
...
function photominiee()
{
	...
	thread PlayFxWithCleanup(level._effect["poltergeist"], level.photocenter.origin);
	thread PlayFxWithCleanup(level._effect["poltergeist"], level.photoright.origin);
	thread PlayFxWithCleanup(level._effect["poltergeist"], level.photoleft.origin);
	wait 0.1;
	...
	thread PlayFxWithCleanup(level._effect["poltergeist"], level.photoleft.origin);
	//testing new fx here
	thread PlayFxWithCleanup(level.newlightingfxaj, level.a19dollatotem.origin);
	wait 0.1;
	level.photoright SetModel("p7_rdj_photo_ee");
	level.photoleft SetModel("p7_19dolla_photo_ee");
	level.photocenter SetModel("p7_bezoz_photo_ee_alt");
...
{
	foreach(spot in level.calendarspots)
	{
		spot Delete();
	}
	...
	level.secretcompartment MoveZ(16, 2);
	wait(2);
	level.secretcompartment PlaySound("vox_153ee_artifactsrequired");
	thread zm_subtitles::subtitle_display(undefined, 2, "-... .- ... . -..", "certain artifacts are required for verification.");
	wait(2);
...
function zuckpillarevent()
{
			...
			thread PlayFxWithCleanup("dlc5/temple/fx_ztem_meteor_shrink", level.zuckpillar.origin, 10);
...
function check153clocks()
{
	...
	while(1)
	{
		...
		if(level.levelclockamount == 28)
		{
			...
			level.clock2 thread deleteallarrayitems();
			level.clock3 thread deleteallarrayitems();
			level.clock4 thread deleteallarrayitems();
			level.clock1hotel thread deleteallarrayitems();
			level.clock2hotel thread deleteallarrayitems();
			level.clock3hotel thread deleteallarrayitems();
...
function a153detectlogica(model)
{
	while(1)
	{
		...
		if(level.clock1int != 4)
		{
				...
				{
					thread PlayFxWithCleanupNotify("dlc5/cosmo/fx_light_red_vista_sm", self.origin, "clockeetimmersucceed");
					self PlaySound("candleignite00");
					level.clock1int ++; 
					break;
				}
				wait(0.15);
...
function a153detectlogicd(model)
{
	while(1)
	{
		...
		//forgot what this did? seems to be depreciated?
...
function deletealljauntsonclock()
{
	foreach(trig in self)
	{
		trig Delete();
	}
}
	...
	while(1)
	{
		if(level.levelclockamount != 0)
		{
			...
			if(level.levelclockamount == 21)
			{
				...
				level.clock1 deletealljauntsonclock();
				level.clock2 deletealljauntsonclock();
				level.clock3 deletealljauntsonclock();
				level.clock4 deletealljauntsonclock();
				level.clock1hotel deletealljauntsonclock();
				level.clock2hotel deletealljauntsonclock();
				level.clock3hotel deletealljauntsonclock();
				level.clock1 Delete();
				level.clock2 Delete();
				level.clock3 Delete();
				level.clock4 Delete();
