...
function main()
{
	...
	while(1)
	{
		...
		if( isDefined(level.tele_cost) )
			while(1)
			{
				...
				if( isDefined(level.tele_cost) && player.score >= level.tele_cost )
				{
					...
					PlayFX(level.teleportbeamfxjaunt, level.teleport_trigger.origin);
					//wait(40);
		...
		PlayFX(KINO_SWIRL2, level.teleport_trigger.origin);
		PlayFX(KINO_SWIRL2, level.mainframetelefx.origin);
				...
				if( players[i] isTouching(level.teleportdetectplayerzombies) )
				{
					...
					if(players[i].quedtoteleport != true)
					{
						...
						if(chance == level.chance_to_rand )
						{
								...
								players[i] thread tele_to_temp_room( level.time_in_pap, i ); 
								//teleporter_sounds(); 	
						}
						else 
						{
		...
		if(level.aresoulcanistersfilledee == true)
		{
			...
			if(teleportingplayercount == players.size)
			{
				...
				for(i = 0; i < players.size; i++)
				{
					players[i] thread move_to_room_ee_roof(temproofspots, i);
				}
...
{ 
	...
	wait(0.05); 
	PlayFX(level.teleportbeamfxjaunt, self.origin);
...
function tele_to_room( time, num )
{
	...
	PlayFX(level.playerteleportfxplayer, self.origin);
...
function telebeamfx()
{
	PlayFX(level.teleportbeamfxjaunt, level.finaltelepadmodel.origin);
}
